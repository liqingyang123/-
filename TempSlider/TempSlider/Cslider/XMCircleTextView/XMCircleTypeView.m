...
- (void)drawRect:(CGRect)rect
{
    ...
    if(self.isCustomized == 0){
        ...
        for (NSInteger charIdx=self.text.length-1; charIdx>=0; charIdx--) {
            ...
            CGContextSaveGState(context);
            CGContextTranslateCTM(context, characterPoint.x, characterPoint.y);
            CGAffineTransform textTransform = CGAffineTransformMakeRotation(angle + M_PI_2*3);
            CGContextConcatCTM(context, textTransform);
            CGContextTranslateCTM(context, -characterPoint.x, -characterPoint.y+kHeight(15));
